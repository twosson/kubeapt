...
BASE_URL="https://api.github.com/repos/twosson/kubeapt/releases"
GITHUB_URL="https://github.com/twosson/kubeapt/releases"
...
name="apt_${version}_Windows-64bit.zip"
...
#url64="https://github.com/twosson/kubeapt/releases/download/$(make version)/apt_$(make version | cut -c2-)_Windows-64bit.zip"
#
#checksum64=$(curl --fail -L "${url64}" | shasum -a 256 - | cut -f 1 -d " ")

echo "Updating choco metadata..." >&2
sed -i.bak "s/<version>.*<\/version>/<version>${version}<\/version>/" ./choco/apt.nuspec
