...
type TablePrinter interface {
	PrintTable(obj runtime.Object, options printers.PrintOptions) (*metav1beta1.Table, error)
}
...
func NewHumanReadablePrinter(decoder runtime.Decoder, options printers.PrintOptions) *HumanReadablePrinter {
...
func ValidateRowPrintHandlerFunc(printFunc reflect.Value) error {
	...
	if funcType.In(1) != reflect.TypeOf((*printers.PrintOptions)(nil)).Elem() ||
...
func ValidatePrintHandlerFunc(printFunc reflect.Value) error {
	...
	if funcType.In(1) != reflect.TypeOf((*io.Writer)(nil)).Elem() ||
		funcType.In(2) != reflect.TypeOf((*printers.PrintOptions)(nil)).Elem() ||
...
func PrintTable(table *metav1beta1.Table, output io.Writer, options printers.PrintOptions) error {
...
// be decorated.
func DecorateTable(table *metav1beta1.Table, options printers.PrintOptions) error {
...
func (h *HumanReadablePrinter) PrintTable(obj runtime.Object, options printers.PrintOptions) (*metav1beta1.Table, error) {
	t := reflect.TypeOf(obj)
...
// or an error, if any.
func printRowsForHandlerEntry(output io.Writer, handler *handlerEntry, obj runtime.Object, options printers.PrintOptions, includeHeaders bool) error {
...
func printRows(output io.Writer, rows []metav1beta1.TableRow, options printers.PrintOptions) {
...
func printUnstructured(unstructured runtime.Unstructured, w io.Writer, additionalFields []string, options printers.PrintOptions) error {
...
func labelValues(itemLabels map[string]string, opts printers.PrintOptions) []string {
	var values []string
...
func appendLabelCells(values []interface{}, itemLabels map[string]string, opts printers.PrintOptions) []interface{} {
	for _, key := range opts.ColumnLabels {
		values = append(values, itemLabels[key])
	}
	if opts.ShowLabels {
...
func printObjectMeta(obj runtime.Object, options printers.PrintOptions) ([]metav1beta1.TableRow, error) {
	...
	row.Cells = append(row.Cells, m.GetName(), translateTimestamp(m.GetCreationTimestamp(), nil))
